...
export class Game2048 {
  ...
  BindRender(render: IGame2048Render): void {
    this.OnTilesUpdated.RegisterObserver(e => {
      render.OnTilesUpdated(e);
    });
    this.OnGameFinished.RegisterObserver(() => render.OnGameFinished());

    render.OnTurnAnimationsCompleted.RegisterObserver(() =>
  ...
  private processAction(move: Direction) {
    CommonTools.ConsoleLog("start process action", [
      this.Grid.Serialize(),
      Direction[move]
    ]);
    ...
    if (gameEvents.length > 0) {
      ...
      this.OnTilesUpdated.NotifyObservers(
        new TileCreatedEvent(newTile, newTile.Value)
      );
    } else {
      this.OnTilesUpdated.NotifyObservers(undefined); // Dummy event - just indicator that user made his action without movements
      ...
      if (availTitles.length == 0) {
        ...
        if (!weHaveSomePossibleEvents) {
          // Game is over, dude
          this.OnGameFinished.NotifyObservers(undefined);
        }
      }
    }
